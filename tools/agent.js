...
function buildState(obs) {
	...
	// THE VOLATILE WORD. Decoded in tools/lib/volatiles.js so the singles brain
	// and the doubles one cannot drift apart -- the bug that started this was
	// buildState writing `volatiles.confusion` while the engine reads
	// `volatiles.confused`, so the confusion reading never worked at all.
	VOL.applyTo(me, obs.me.status2);
	VOL.applyTo(foe, obs.foe.status2);
