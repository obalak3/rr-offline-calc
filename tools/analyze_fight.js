...
// The node log is overwritten on every agent restart, so a run's log only
// survives if it was copied into that run's turns dir (agent-node.log there).
let nodeLog = '';
try { nodeLog = fs.readFileSync(path.join(dir, 'agent-node.log'), 'utf8'); }
catch (e) { try { nodeLog = fs.readFileSync(path.join(process.env.HOME, 'rr-agent/agent-node.log'), 'utf8'); } catch (e2) { nodeLog = ''; } }
const resolved = {};
...
let lastPlan = null, lastTurn = null, deaths = [], turns = 0, switches = 0, planChanges = 0, noPlan = 0;
for (const f of files) {
	...
	if (plan !== lastPlan && lastPlan !== null && !/forced/.test(o.kind)) { console.log('   ** PLAN CHANGED'); planChanges++; }
	if (o.kind !== 'forced') { turns++; if (/^switch/.test(String(r.played))) switches++; if (plan === '(no plan)') noPlan++; }
...
console.log('CHURN: ' + switches + ' voluntary switches in ' + turns + ' decided turns ('
	+ (turns ? Math.round(100 * switches / turns) : 0) + '%), plan changed ' + planChanges + ' times, '
	+ noPlan + ' turns with no plan');
